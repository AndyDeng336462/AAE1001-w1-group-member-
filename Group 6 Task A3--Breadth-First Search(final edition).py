        ...
        while open_set:
            ...
            for i, _ in enumerate(self.motion):
                node = self.Node(current.x + self.motion[i][0],
                                 ...
                                 current.cost + self.motion[i][2] * self.costPerGrid, self.calc_grid_index(current))
                ...
                if n_id not in [self.calc_grid_index(n) for n in open_set]:
...
def main():
    print(__file__ + " start the BFS algorithm demo !!")
    ...
    bfs = BFSPlanner(ox, oy, grid_size, robot_radius, fc_x, fc_y, tc_x, tc_y)
